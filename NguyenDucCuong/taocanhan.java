...
public class taocanhan extends javax.swing.JFrame {
    ...
    private void btnCnMoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCnMoActionPerformed
        ...
        if ((txtCNCMND.getText().equals("")) || (txtCNTen.getText().equals("")) || (txtCNSDT.getText().equals(""))
            ...
            String tx1 = txtCNCMND.getText();
            boolean x1 = true;
            for (int i = 0; i < tx1.length(); i++) {
                if ((tx1.charAt(i) != '0') && (tx1.charAt(i) != '1') && (tx1.charAt(i) != '2') && (tx1.charAt(i) != '3')
                        && (tx1.charAt(i) != '4') && (tx1.charAt(i) != '5') && (tx1.charAt(i) != '6') && (tx1.charAt(i) != '7')
                        && (tx1.charAt(i) != '8') && (tx1.charAt(i) != '9')) {
                    x1 = false;
                }
            }
            if ((x1 == false) || ((tx1.length() != 12) && (tx1.length() != 8))) {
                JOptionPane.showMessageDialog(rootPane, "Nhập sai số CMND!!!");
            } else {
                String tx2 = txtCNSDT.getText();
                boolean x2 = true;
                for (int i = 0; i < tx2.length(); i++) {
                    if ((tx2.charAt(i) != '0') && (tx2.charAt(i) != '1') && (tx2.charAt(i) != '2') && (tx2.charAt(i) != '3')
                            && (tx2.charAt(i) != '4') && (tx2.charAt(i) != '5') && (tx2.charAt(i) != '6') && (tx2.charAt(i) != '7')
                            && (tx2.charAt(i) != '8') && (tx2.charAt(i) != '9')) {
                        x2 = false;
                    }
                }
                if (x2 == false) {
                    JOptionPane.showMessageDialog(rootPane, "Nhập sai số điện thoại!!!");
                } else {
                    String tx3 = txtCNHM.getText();
                    boolean x3 = true;
                    for (int i = 0; i < tx3.length(); i++) {
                        if ((tx3.charAt(i) != '0') && (tx3.charAt(i) != '1') && (tx3.charAt(i) != '2') && (tx3.charAt(i) != '3')
                                && (tx3.charAt(i) != '4') && (tx3.charAt(i) != '5') && (tx3.charAt(i) != '6') && (tx3.charAt(i) != '7')
                                && (tx3.charAt(i) != '8') && (tx3.charAt(i) != '9')) {
                            x3 = false;
                        }
                    }
                    if (x3 == false) {
                        JOptionPane.showMessageDialog(rootPane, "Nhập sai hạn mức tín dụng!!!");
                    } else {
                        boolean xett = true;
                        DB_hskh gg = new DB_hskh();
                        for (hosokhachhang j : gg.layhoso()) {
                            if (j.getMaHS().equals(tx1)) {
                                xett = false;
                                
                            }
                        }
                        if (xett == false) {
                            JOptionPane.showMessageDialog(rootPane, "Số CMND này đã tồn tại");
                        } else {
                            try {
